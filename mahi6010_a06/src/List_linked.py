...
class List:
    ...
    def _linear_search_r(self, key):
        """
        -------------------------------------------------------
        Searches for the first occurrence of key in list using recursion.
        Private helper method.
        (iterative algorithm)
        Use: previous, current, index = self._linear_search(key)
        -------------------------------------------------------
    ...
    def _linear_search_r_aux(self, key, previous, current, index):
        ...
        if current is None:
            ...
            index += 1
            previous, current, index = self._linear_search_r_aux(
                key, current, current._next, index)
    ...
    def is_identical_r(self, other):
        ...
        if self._count != other._count:
            ...
            source_node, other_node = self._is_identical_r_aux(
    ...
    def _is_identical_r_aux(self, source_node, other_node):
        ...
        if source_node is not None and source_node._value == other_node._value:
            source_node, other_node = self._is_identical_r_aux(
                source_node._next, other_node._next)
